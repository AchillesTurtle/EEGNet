...
trial_n = int(1.3 * downsample_freq)
amp=2*np.sqrt(2)
"""
...
train_data = np.empty([5440, 1, chan_num, 260], dtype=float)
test_data = np.empty([3400, 1, chan_num, 260], dtype=float)
...
for itera, sample in enumerate(train_sample_num):
    for test_num in range(1, 6):
        ...
        stft_data=np.empty([int(len(data[:,0])*128/200),58])
        for n in range(int(len(data[:,0])*128/200)):
            stft_data[n,:] = filtered_data[int(n * 200 / downsample_freq),0:58]
        f,t,Zstft=stft(stft_data,downsample_freq,nperseg=128,axis=0) #length of event?
        print(Zstft.shape)
        print(t)
        ...
        print(f.shape)
        print(t.shape)
        plt.pcolormesh(t, f, np.abs(Zstft[:,30,:]), vmin=0, vmax=2)
        plt.colorbar()
        ...
        for index, event_stamp in enumerate(EventStampList):
            segment = np.reshape(np.transpose(filtered_data[Zstft:Zstft + trial_n, chan_index]), (1, 1, chan_num, trial_n))
            """
...
for itera, sample in enumerate(test_sample_num):
    for test_num in range(1, 6):
        ...
        for index, EventStamp in enumerate(data[:, 58]):  # index in excel minus 2
            if EventStamp:
                EventStampList.append((int)(index*200/downsample_freq))
        ...
        for n in range(len(data[:,0])):
            filtered_data[n,:] = filtered_data[int(n * 200 / downsample_freq),:]
        f,t,Zstft=stft(filtered_data,downsample_freq,nperseg=128) #length of event?
        # extract and shape data as input
...
if downsample_flag:
    ...
    downsampled_train_data = np.empty([5440, 1, chan_num, trial_n], dtype=float)
    for index in range(5440):
        for n in range(trial_n):
            downsampled_train_data[index, 0, :, n] = train_data[index, 0, :, int(n * 200 / downsample_freq)]
    downsampled_test_data = np.empty([3400, 1, chan_num, trial_n], dtype=float)
    for index in range(3400):
        for n in range(trial_n):
            downsampled_test_data[index, 0, :, n] = test_data[index, 0, :, int(n * 200 / downsample_freq)]
    downsampled_train_data = downsampled_train_data.reshape(5440 * chan_num, trial_n)
    downsampled_test_data = downsampled_test_data.reshape(3400 * chan_num, trial_n)
    np.savetxt("56_norm_train_data_filtered_downsampled %d.csv" % downsample_freq, downsampled_train_data, delimiter=",")
    np.savetxt("56_norm_test_data_filtered_downsampled %d.csv" % downsample_freq, downsampled_test_data, delimiter=",")
    print('Down sampled to %d' % downsample_freq)
else:
    train_data = train_data.reshape(5440 * chan_num, 260)
    test_data = test_data.reshape(3400 * chan_num, 260)
    np.savetxt("56_norm_train_data_filtered.csv", train_data, delimiter=",")
    np.savetxt("56_norm_test_data_filtered.csv", test_data, delimiter=",")
